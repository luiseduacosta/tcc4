...
class EstagiariosController extends AppController
{
    ...
    public function index($id = null)
    {
        ...
        foreach ($instituicoes as $instituicao) {
            if (!empty($instituicao->instituicao->id)) {
                $listainstituicoes[$instituicao->instituicao->id] =
        ...
        foreach ($instituicoes as $supervisor) {
            if (!empty($supervisor->supervisor->id)) {
                $listasupervisores[$supervisor->supervisor->id] =
        ...
        foreach ($instituicoes as $professor) {
            if (!empty($professor->professor->id)) {
                $listaprofessores[$professor->professor->id] =
        ...
        foreach ($instituicoes as $turmaestagio) {
            if (!empty($turmaestagio->turmaestagio->id)) {
                $listaturmaestagios[$turmaestagio->turmaestagio->id] =
                    $turmaestagio->turmaestagio->area;
    ...
    public function novotermocompromisso($id = null)
    {
        ...
        if ($aluno_id === null) {
            $this->Flash->error(
                __("Selecionar o aluno para o termo de compromisso"),
            );
            return $this->redirect([
                "controller" => "Alunos",
                "action" => "index",
            ...
            if ($estagiario) {
                ...
                if ($estagiario->periodo == $periodoatual) {
                    return $this->redirect([
                        ...
                        "action" => "edit",
                        $estagiario->id,
                    ]); // Redireciona para a atualização do estagiario
                } else {
                    return $this->redirect([
                        "controller" => "Estagiarios",
                        "action" => "add",
    ...
    public function termodecompromissopdf($id = null)
    {
        ...
        if ($id === null) {
            throw new \Cake\Http\Exception\NotFoundException(
                __("Sem parâmetros para localizar o estagiário"),
            );
        } else {
            $estagiario = $this->Estagiarios
                ->find()
    ...
    public function declaracaodeestagiopdf($id = null)
    {
        ...
        if (empty($estagiario->aluno->orgao)) {
            $this->Flash->error(
                __("Estudante não especifica o orgão emisor do documento"),
            );
            return $this->redirect("/Alunos/view/" . $estagiario->aluno->id);
        }
    ...
    public function edit($id = null)
    {
        if ($id === null) {
            $this->Flash->error(
                __("Sem parâmetro para localizar o estagiário."),
            );
            return $this->redirect(["action" => "index"]);
        }
        $this->Authorization->skipAuthorization();
        ...
        try {
        $estagiario = $this->Estagiarios->get($id, contain: ['Alunos', 'Instituicoes', 'Professores', 'Supervisores', 'Turmaestagios'],);
        ...
        if ($this->request->is(["patch", "post", "put"])) {
            ...
            $this->Flash->error(
                __(
                    "Registro de estagiario nao foi atualizado. Tente novamente.",
                ),
            );
        }
    ...
    public function lancanota($id = null)
    {
        ...
        if ($siape === null) {
            $this->Flash->error(
                __(
                    "Sem parâmetro para localizar o estagiário do(a) professor(a).",
                ),
            );
            return $this->redirect(["action" => "index"]);
        }
        $estagiarios = $this->Estagiarios->Professores
            ->find()
            ->contain([
                "Estagiarios" => [
                    "sort" => ["periodo" => "desc"],
        ...
        foreach ($estagiarios as $estagiario):
            foreach ($estagiario as $c_estagio):
                ...
                $estagiarioslancanota[$i]["instituicao_id"] =
                    $c_estagio["instituicao"]["id"];
                $estagiarioslancanota[$i]["instituicao"] =
                    $c_estagio["instituicao"]["instituicao"];
                $estagiarioslancanota[$i]["supervisor_id"] =
                    $c_estagio["supervisor"]["id"];
                $estagiarioslancanota[$i]["supervisora"] =
                    $c_estagio["supervisor"]["nome"];
                $estagiarioslancanota[$i]["professor_id"] =
                    $c_estagio["docente"]["id"];
                $estagiarioslancanota[$i]["docente"] =
                    $c_estagio["docente"]["nome"];
                $estagiarioslancanota[$i]["estudante_id"] =
                    $c_estagio["aluno"]["id"];
                $estagiarioslancanota[$i]["estudante"] =
                    $c_estagio["aluno"]["nome"];
                if (isset($c_estagio["avaliacao"]["id"])):
                    $estagiarioslancanota[$i]["avaliacao_id"] =
                        $c_estagio["avaliacao"]["id"];
                else:
