...
$supervisora = isset($estagiario->estagiario->supervisor->nome);
if ($supervisora) {
    $supervisora = $estagiario->estagiario->supervisor->nome;
} else {
    $supervisora = "____________________";
}

$regiao = isset($estagiario->estagiario->supervisor->regiao);
if ($regiao) {
    $regiao = $estagiario->estagiario->supervisor->regiao;
} else {
    $regiao = '__';
}

$cress = isset($estagiario->estagiario->supervisor->cress);
if ($cress) {
    $cress = $estagiario->estagiario->supervisor->cress;
} else {
    $cress = '_____';
}

$instituicao = isset($estagiario->estagiario->instituicaoestagio->instituicao);
if ($instituicao) {
    $instituicao = $estagiario->estagiario->instituicaoestagio->instituicao;
} else {
    $instituicao = '_______________';
}

$professora = isset($estagiario->estagiario->docente->nome);
if ($professora) {
    $professora = $estagiario->estagiario->docente->nome;
} else {
...
<h2 style="text-align:center; line-height: 80%; margin: 0">
    ...
    <br />
    <span style="font-size: 100%">Folha de ativiades do(a) estagiário(a) <?= $estagiario->estagiario->estudante->nome ?></span>
</h2>

<p style="font-size: 90%">DRE: <?= $estagiario->estagiario->estudante->registro ?>
    Telefone: <?= $estagiario->estagiario->estudante->celular ?>
    E-mail: <?= $estagiario->estagiario->estudante->email ?>
</p>
...
<div class="container">
    ...
    <table class="table" style="width: 100%; background-color: white;">
        <tr>
            ...
            <td style="width: 33%"><span style="font-size: 100%; text-decoration: overline"><?= $estagiario->estagiario->estudante->nome ?></span></td>
            <td style="width: 33%"><span style="font-size: 100%; text-decoration: overline"><?= $supervisora ?></span></td>
        </tr>

        <tr>
            <td style="width: 33%"></td>
            <td style="width: 33%"><span style="font-size: 100%">DRE: <?= $estagiario->estagiario->estudante->registro ?></span></td>
