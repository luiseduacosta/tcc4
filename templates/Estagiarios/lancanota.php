                ...
                <?php foreach ($estagiarios as $estagiario): ?>
                    ...
                    <tr>
                        <?php if ($this->getRequest()->getSession()->read('id_categoria') == 1): ?>
                            <td><?= $estagiario->id ?></td>
                        <?php endif; ?>
                        <td><?= $this->Html->link($estagiario['estudante'], ['controller' => 'Estudantes', 'action' => 'view', $estagiario['estudante_id']]) ?></td>
                        <td><?= $estagiario['registro'] ?></td>
                        <td><?= $this->Html->link($estagiario['instituicao'], ['controller' => 'Instituicaoestagios', 'action' => 'view', $estagiario['instituicao_id']]) ?></td>
                        <td><?= $this->Html->link($estagiario['supervisora'], ['controller' => 'Supervisores', 'action' => 'view', $estagiario['supervisor_id']]) ?></td>
                        ...
                        <?php if (isset($estagiario['folha_id'])): ?>
                            <td><?= $this->Html->link('Folha de atividades', ['controller' => 'Folhadeatividades', 'action' => 'index', $estagiario['id']]) ?></td>
                        <?php else: ?>
                            <td></td>
                        <?php endif; ?>
                        <?php if (isset($estagiario['avaliacao_id'])): ?>
                            <td><?= $this->Html->link('Ver avaliação', ['controller' => 'avaliacoes', 'action' => 'view', $estagiario['avaliacao_id']]) ?></td>
                        <?php else: ?>
                            <td></td>
                        <?php endif; ?>
                        <td class="actions">
